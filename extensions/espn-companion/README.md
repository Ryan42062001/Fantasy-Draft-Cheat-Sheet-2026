...
- If Captured is greater than Applied, refresh the War Room tab and press **Rescan ESPN**. Version 0.8.10 caps screen picks at the configured draft total, retains the last successful structured snapshot when ESPN closes a completed mock feed, identifies stale website-version reports, and provides a fallback when Chrome's clipboard API is unavailable. It also lets the website request a fresh ESPN PPR board-rank and ADP scan. FantasyPros ECR remains the value authority.
