...
Version 0.9.10 observes structured data ESPN's own page receives through WebSocket, fetch, XHR, EventSource, and bounded React-state inspection. A unified ledger reconciles those observations by overall pick and ESPN player ID. ESPN's REST draft-detail response is retained as a snapshot/recovery source, and visible Pick History/Board parsing remains the final fallback. This architecture is implemented and replay-tested, but structured live capture still requires a disposable live-mock validation before it should be relied on for a real draft.
...
Version 0.9.10 uses the documented 2026 expert endpoint to identify currently publishing members of FantasyPros' finalized 2025 Draft Accuracy Top-20 preset. It reads `accuracy_draft.ALL` when supplied and falls back to the official finalized Top-20 names when the live directory omits that per-expert field; transient expert IDs are never hard-coded. The parser accepts the documented array or FantasyPros' live expert-ID-keyed object shape. The live FantasyPros page produced a 10-expert PPR consensus from that preset during validation on 2026-08-24. The extension verifies `accuracy_draft_season`, caches the active subset for 24 hours, and accepts 1–20 active contributors. The first refresh after cache expiry uses two requests; later refreshes use one consensus request. Each attempt stores credential-safe stage, HTTP, payload-shape, expert, player-validation, request-count, and War Room delivery diagnostics with an explicit next step. The key is stored separately in `chrome.storage.local`; it is never returned by status requests, copied diagnostics, or War Room snapshots. The update is applied only when the API returns active preset contributors and 100–600 unique, position-valid players. CSV import remains available as fallback.
